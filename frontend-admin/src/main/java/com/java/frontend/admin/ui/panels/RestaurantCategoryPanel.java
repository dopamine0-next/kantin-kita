...
import com.java.frontend.admin.service.RestaurantCategoryService;
...
public class RestaurantCategoryPanel extends JPanel {

    private final DefaultTableModel tableModel;
    private final JTable table;
    private List<AdminRestaurantCategory> data;
    private final Gson gson = new Gson();
    private JLabel errorLabel;
    private JPanel tableContainer;

    public RestaurantCategoryPanel() {
    ...
    private void loadData() {
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() {
                try {
                    data = RestaurantCategoryService.findAll();
                    SwingUtilities.invokeLater(() -> {
                        tableModel.setRowCount(0);
                        for (AdminRestaurantCategory c : data) {
    ...
    private AdminRestaurantCategory selectedCategory() {
        int row = selectedRow();
    ...
    private void tambah() {
        ...
        FormDialog dlg = new FormDialog((JFrame) SwingUtilities.getWindowAncestor(this), "Tambah Kategori", fields);
        dlg.setVisible(true);
    ...
    private void edit() {
        AdminRestaurantCategory c = selectedCategory();
        if (c == null) return;
        ...
        FormDialog dlg = new FormDialog((JFrame) SwingUtilities.getWindowAncestor(this), "Edit Kategori", fields);
        dlg.setVisible(true);
    ...
    private void callCreate(String json) {
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() {
                try {
                    RestaurantCategoryService.create(json);
                } catch (Exception ex) {
                    error = ex.getMessage();
                }
                return null;
            ...
            protected void done() {
                if (error != null) {
                    JOptionPane.showMessageDialog(RestaurantCategoryPanel.this, "Gagal: " + error);
                } else {
                    loadData();
                }
            }
    ...
    private void callUpdate(String id, String json) {
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() {
                try {
                    RestaurantCategoryService.update(id, json);
                } catch (Exception ex) {
                    error = ex.getMessage();
                }
                return null;
    ...
    private void hapus() {
        ...
        new SwingWorker<Void, Void>() {
            ...
            protected Void doInBackground() {
                try {
                    RestaurantCategoryService.delete(c.getId());
                } catch (Exception ex) {
                    error = ex.getMessage();
                }
                return null;
